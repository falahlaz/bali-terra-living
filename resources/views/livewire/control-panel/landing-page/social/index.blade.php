    ...
    <x-card>
        ...
        <x-card.body>
            <div class="flex justify-end mb-3">
                <x-button
                    :href="route('cp.social-links.create')"
                    with_icon="true"
                    width="12"
                    height="12"
                    view-box="0 0 24 24"
            ...
            <x-tables.base>
                <x-tables.header>
                    ...
                    <x-tables.cells.header title="Platform"/>
                    <x-tables.cells.header title="URL"/>
                    <x-tables.cells.header title="Icon"/>
                ...
                <x-tables.body>
                    @foreach($social_links as $social_link)
                        <tr>
                            ...
                            <x-tables.cells.body :data="$social_link->platform"/>
                            <x-tables.cells.body :data="$social_link->url"/>
                            <x-tables.cells.body :data="$social_link->icon"/>
                            <x-tables.cells.body-with-variant :variant="$social_link->is_active ? 'success' : 'danger'" :data="$social_link->is_active ? 'Active' : 'Inactive'"/>
                            <x-tables.cells.body>
                                <x-button
                                    :href="route('cp.social-links.detail', [$social_link->id])"
                                    with_icon="true"
                                    variant="primary"
                                    width="12"
                                    height="12"
