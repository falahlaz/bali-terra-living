        ...
        <x-card.body>
            ...
            <x-tables.base>
                <x-tables.header>
                    ...
                    <x-tables.cells.header title="Parent"/>
                    <x-tables.cells.header title="Label"/>
                ...
                <x-tables.body>
                    @foreach($menus as $menu)
                        <tr>
                            ...
                            <x-tables.cells.body :data="$menu->menu_location->asTitle()"/>
                            <x-tables.cells.body :data="$menu->parent?->label ?? '-'"/>
                            <x-tables.cells.body :data="$menu->label"/>
                            <x-tables.cells.body :data="$menu->url"/>
                            <x-tables.cells.body-with-variant :variant="$menu->is_active ? 'success' : 'danger'" :data="$menu->is_active ? 'Active' : 'Inactive'"/>
                            <x-tables.cells.body>
                                <x-button
                                    :href="route('cp.menus.detail', [$menu->id])"
                                    with_icon="true"
                                    variant="primary"
                                    width="12"
                                    height="12"
