                ...
                <x-tables.body>
                    @foreach($about_cards as $about_card)
                        <tr>
                            ...
                            <x-tables.cells.body :data="$about_card->title"/>
                            <x-tables.cells.body-with-variant :variant="$about_card->is_active ? 'success' : 'danger'" :data="$about_card->is_active ? 'Active' : 'Inactive'"/>
                            <x-tables.cells.body>
                                <x-button
                                    :href="route('cp.about.detail', [$about_card->id])"
                                    with_icon="true"
                                    variant="primary"
                                    width="12"
                                    height="12"
