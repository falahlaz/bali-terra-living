                ...
                <x-tables.body>
                    @foreach($benefits as $benefit)
                        <tr>
                            ...
                            <x-tables.cells.body :data="$benefit->title"/>
                            <x-tables.cells.body-with-variant :variant="$benefit->is_active ? 'success' : 'danger'" :data="$benefit->is_active ? 'Active' : 'Inactive'"/>
                            <x-tables.cells.body>
                                <x-button
                                    :href="route('cp.benefits.detail', [$benefit->id])"
                                    with_icon="true"
                                    variant="primary"
                                    width="12"
                                    height="12"
