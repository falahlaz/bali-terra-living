<x-landing-page.layouts.app>
    ...
    <x-landing-page.sections id="contact">
        ...
        <form action="{{route('contact.store')}}" method="POST"
              class="bg-white p-10 lg:p-12 rounded-2xl shadow-lg space-y-6">
            @csrf
            @method('POST')
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    @if($errors->has('full_name'))
                        <p class="text-red-500 text-sm mt-1 italic">{{$errors->first('full_name')}}</p>
                    @endif
                </div>
                <div>
                    ...
                    @if($errors->has('email'))
                        <p class="text-red-500 text-sm mt-1 italic">{{$errors->first('email')}}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input name="phone_number" type="tel" id="phone" value="{{old('phone_number')}}"
                           class="w-full px-5 py-4 rounded-2xl border-2 border-sage/20 focus:border-sage focus:outline-none transition-colors"
                           placeholder="+62 123 456 7890">
                    @if($errors->has('phone_number'))
                        <p class="text-red-500 text-sm mt-1 italic">{{$errors->first('phone_number')}}</p>
                    @endif
                </div>
                <div>
                    <label for="interest" class="block text-earth font-medium mb-3">I'm Interested In</label>
                    <select name="interested_in" id="interest"
                            class="w-full px-5 py-4 rounded-2xl border-2 border-sage/20 focus:border-sage focus:outline-none transition-colors">
                        <option value="">-- Select --</option>
                        <option @checked(old('interested_in', 'Buying a Villa')) value="Buying a Villa">Buying a
                            Villa
                        </option>
                        <option
                            @checked(old('interested_in', 'Investment Opportunities')) value="Investment Opportunities">
                            Investment Opportunities
                        </option>
                        <option @checked(old('interested_in', 'Property Management')) value="Property Management">
                            Property Management
                        </option>
                    </select>
                    @if($errors->has('interested_in'))
                        <p class="text-red-500 text-sm mt-1 italic">{{$errors->first('interested_in')}}</p>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                @if($errors->has('message'))
                    <p class="text-red-500 text-sm mt-1 italic">{{$errors->first('message')}}</p>
                @endif
            </div>
